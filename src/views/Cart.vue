...
//        filters:{////########
//					// 因为我们引入currency文件，是一个函数，所以currency就是函数
//          currency:currency
//        },
        computed:{ // computed里面的值都是实时计算的
          checkAllFlag(){ //实时计算
...
//                    this.$store.commit("updateCartCount",-delCount);
                    this.init();// 数据发生变化，所以要重新加载数据，但多了一次请求
                    // 另一种方式：
                    // 因为现在是已经修改成功了，所以我们利用prudctId来对this.cartList这个数组来
                    // 进行删除，这样就少调用一次接口(this.init())
                  }
              });
            },
            editCart(flag,item){ ///////
                if(flag=='add'){
                  ...
                  item.checked = item.checked=="1"?'0':'1';//就是取反
                }
								// 改变前端的productNum，要使数据和后台同步
