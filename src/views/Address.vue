                ...
                <ul>
                  ...
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                    ...
                    <div class="addr-opration addr-set-default">
											<!--点击之后默认地址和 设置默认地址-->
                      <a href="javascript:;"  v-if="!item.isDefault" @click="setDefault(item.addressId)"><i>设置默认地址</i></a>
                    </div>
...
<script>
  ...
  export default{
      ...
      methods:{
          ...
          expand(){ // 展开地址
              if(this.limit==3){
                this.limit = this.addressList.length; // 展开，则显示全部
              }else{
                this.limit = 3; // 收起，再次显示三条
              }
          },
